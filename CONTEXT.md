...
- The **OpenCode Client** uses plural consumer-facing capability groups such as `sessions`. Internal server identifiers do not implicitly define public client names.
...
- Promise methods accept a separate optional per-call transport-options argument containing `AbortSignal` and header overrides. Cancellation and transport metadata do not enter the domain input object.
...
- The Effect-native scoped host belongs to `@opencode-ai/sdk-next`. Client remains network-only and SDK depends one-way on Client.
...
- The common OpenCode Client does not expose server-global event aggregation. `events.subscribe()` is bounded to the connected OpenCode instance or workspace.
- OpenCode initializes one server-wide Ed25519 deploy key under its global data directory. `deployKeys.get()` exposes its public key and managed file paths; private key material never crosses the HTTP boundary.
...
Additional public schemas stay in Schema and additional network groups stay in Protocol; neither package may transitively load databases, Drizzle, Session execution, providers, watchers, native modules, or WASM. Concrete Location middleware keys stay in Server while Protocol owns their placement. Hosts that share durable Session storage must also share process-local Session execution coordination, or each host must receive isolated storage explicitly. An embedded request scope stays alive until any streamed response body finishes.
