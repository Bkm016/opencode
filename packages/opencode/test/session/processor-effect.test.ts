    ...
    stream: () =>
      Stream.make(
        ...
        LLMEvent.toolResult({
          id: "image-1",
          name: "image_generation",
          result: { type: "json", value: { result: generatedPng } },
          providerExecuted: true,
        }),
        LLMEvent.stepFinish({ index: 0, reason: "stop" }),
        LLMEvent.finish({ reason: "stop" }),
...
const imageGenerationEnv = LayerNode.compile(root, [...replacements, [LLM.node, imageGenerationLLM]])
  ...
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        ...
        // 上游异常断流：只吐出部分正文就结束 SSE，既无 finish_reason 也无 usage。
        yield* llm.push(reply().text("partial").item())
        yield* llm.text("recovered", { usage: { input: 10, output: 5 } })

        const chat = yield* session.create({})
        const parent = yield* user(chat.id, "truncated stream")
        const msg = yield* assistant(chat.id, parent.id, path.resolve(dir))
        const mdl = yield* provider.getModel(ref.providerID, ref.modelID)
        const handle = yield* processors.create({
          assistantMessage: msg,
...
itImageGeneration.live("session.processor stores OpenAI generated images as tool attachments", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()
