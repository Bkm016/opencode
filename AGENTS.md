...
- To build the Windows desktop app, run `bun ./script/build-windows.ts` (use `--skip-sidecar` for UI-only changes to speed up). Artifacts are placed in `packages/desktop/dist/`.
...
- Keep `SessionExecution` process-global and Session-ID based. Default wiring is `SessionExecution.noopLayer` (admit-only); no core SessionRunner/V2 tool stack is present. Production chat execution stays on the V1 `SessionPrompt` path in `packages/opencode`.
- Keep permissions and filesystem Location-scoped. Omitted `Location.workspaceID` means implicit-local placement.
- Keep EventV2 replay owner claims separate from Session execution ownership.
