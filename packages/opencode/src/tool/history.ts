 ...
 * source=task 是显式例外，只能回查当前任务已持久化授权的祖先片段，不接受任意 Session ID。
 */

const GrepParameters = Schema.Struct({
  source: Schema.optional(Schema.Literals(["session", "task"])).annotate({
    description: "Default session. Use task to search only ancestor evidence authorized in this task handoff.",
  }),
  ...
  message_id: Schema.optional(Schema.String).annotate({
    description: "User message ID for searching an unbounded text part",
  }),
...
const ListParameters = Schema.Struct({
  source: GrepParameters.fields.source,
  chunk_id: Schema.optional(Schema.String).annotate({ description: "8-character chunk display ID" }),
  message_id: Schema.optional(Schema.String).annotate({
    description: "User message ID for reading an unbounded text part",
  }),
  part_id: Schema.optional(Schema.String).annotate({ description: "Text part ID, used with message_id" }),
  offset: Schema.optional(Schema.Number).annotate({
    description:
      "Line offset within the selected chunk or text part (default 0). Without part_id, offset pages across the message's text parts.",
  }),
...
const taskEvidence = Effect.fn("History.taskEvidence")(function* (
  ...
  const metadata = { source: "task", matches: 0, lines: 0, total: 0 }
  const title = params.pattern ?? "task evidence"
  if (params.chunk_id || (params.part_id && !params.message_id)) {
    return {
      title,
      metadata,
      output: "Task evidence uses message_id and part_id, not chunk_id. part_id requires message_id.",
    }
  }
  const evidence = (yield* TaskContext.load(sessions, sessionID)).filter(
    (entry) =>
      (!params.message_id || entry.reference.messageID === params.message_id) &&
      (!params.part_id || entry.reference.partID === params.part_id),
  )
  if (!evidence.length) return { title, metadata, output: "No authorized task evidence matches this reference." }
  if (params.pattern === undefined && (!params.message_id || !params.part_id)) {
    const offset = Math.max(0, Math.trunc(params.offset ?? 0))
    const page = evidence.slice(offset, offset + Math.min(80, Math.max(1, Math.trunc(params.limit ?? 80))))
    return {
      title,
      metadata: { ...metadata, total: evidence.length, lines: page.length },
      output: [
        `Authorized task evidence (${offset}..${offset + page.length} of ${evidence.length}). Supply source=task, message_id and part_id to read; omit both and increase offset to page the reference index:`,
        ...page.map(
          (entry) =>
            `message_id=${entry.reference.messageID} part_id=${entry.reference.partID} ${entry.reference.original ? "ORIGINAL_USER" : "EVIDENCE"}`,
        ),
      ].join("\n"),
    }
  }
  // 用户原文沿用长行分段协议，保证单行 JSON 中部也能通过相同引用和 offset 回查。
  const entries = evidence.flatMap((entry) => {
    const lines =
      entry.info.role === "user"
        ? SessionChunk.userTextTranscript({ messages: [{ info: entry.info, parts: [entry.part] }] }).map((line) => ({
            ...line,
            source: "USER" as const,
          }))
        : SessionChunk.partTranscript(entry.info.role, entry.part)
    return lines.map((line) => ({ ...line, reference: entry.reference }))
  })
  const selected =
    params.pattern === undefined
      ? entries.slice(
          Math.max(0, Math.trunc(params.offset ?? 0)),
          Math.max(0, Math.trunc(params.offset ?? 0)) + Math.min(80, Math.max(1, Math.trunc(params.limit ?? 80))),
        )
      : entries
          .filter((entry) =>
            (params.case_sensitive ? entry.text : entry.text.toLowerCase()).includes(
              params.case_sensitive ? params.pattern! : params.pattern!.toLowerCase(),
            ),
          )
          .slice(0, Math.min(20, Math.max(1, Math.trunc(params.head_limit ?? 20))))
  return {
    title,
    metadata: {
      ...metadata,
      ...
      matches: params.pattern === undefined ? 0 : selected.length,
    },
    output: selected.length
      ? selected
          .map(
            (entry) =>
              `message_id=${entry.reference.messageID} part_id=${entry.reference.partID} line=${entry.line} ${entry.source}:\n${entry.text}`,
          )
          .join("\n\n")
      : "No matches or lines in authorized task evidence.",
  }
})
              ...
              Effect.flatMap((chunkCtx) => {
                ...
                if (params.message_id) {
                      ...
                      return [
                        {
                          ...entry,
                          context: [
                            ...entries.slice(Math.max(0, index - 2), index),
                            ...entries.slice(index + 1, index + 3),
                          ].filter(
                            (neighbor) => neighbor.messageID === entry.messageID && neighbor.partID === entry.partID,
                          ),
                        },
                      ]
                    })
                  ...
                  if (hits.length === 0) {
                    return Effect.succeed({
                      title: params.pattern,
                      metadata: { matches: 0 },
                      output: "No matches in referenced user text.",
                    })
                  }
                  const output = hits
                    .map((hit) => {
                ...
                if (hits.length === 0) {
                  return Effect.succeed({
                    title: params.pattern,
                    metadata: { matches: 0 },
                    output: "No matches in folded history.",
                  })
                }
                const output = hits
                  .map((hit) => {
...
export const HistoryListTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    return {
      description:
        "List a folded chunk or referenced original user text by line range. Use source=task for authorized ancestor evidence; omit message_id and part_id to list its references, then provide both to read. Use history_grep first to locate a line. Output is subject to normal tool truncation.",
      parameters: ListParameters,
      execute: (
        params: Schema.Schema.Type<typeof ListParameters>,
        ctx: Tool.Context,
      ): Effect.Effect<Tool.ExecuteResult<ListMeta>> =>
        params.source === "task"
          ? taskEvidence(sessions, ctx.sessionID, params).pipe(Effect.orDie)
          : loadHistoryCtx(sessions, ctx.sessionID).pipe(
              Effect.flatMap((chunkCtx) => {
