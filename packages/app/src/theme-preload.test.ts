...
describe("theme preload", () => {
  test("migrates legacy oc-1 to claude before mount", () => {
    ...
    expect(localStorage.getItem("opencode-theme-id")).toBe("claude")
    expect(localStorage.getItem("opencode-theme-css-light")).toBeNull()
    expect(localStorage.getItem("opencode-theme-css-dark")).toBeNull()
    expect(document.getElementById("oc-theme-preload")).toBeNull()
  })

  test("migrates legacy oc-2 to claude before mount", () => {
  ...
  test("falls back unknown themes to claude before mount", () => {
  ...
  test("keeps cached css for the default theme", () => {
    localStorage.setItem("opencode-theme-id", "claude")
    localStorage.setItem("opencode-theme-css-light", "--background-base:#fff;")
